...
! 5*rea -> mag_tempo.plt
! 6*rea -> fator_estrutura.plt
! 7*rea -> poincare.plt
! 100*rea-> trajetorias.plt
! 200*rea-> distribuicao_agregados.plt
! 300*rea-> deslocamentos.plt

! Creating a file that will be used to plot the trajectory of an arbitrary test particle

open(100*c,file='trajetorias.plt')
write(100*c,*) 'Variables="X","Y","Z","Dx","Dy","Dz"'


!open(300*c,file='passo_de_tempo.plt')
!write(300*c,*)'Variables="dt","T"'

open(7*rea,file='poincare.plt')
write(7*rea,*) 'Variables="X","Y"'
...
open(300*c,file='deslocamentos.plt')
write(300*c,*)'Variables="<x>","<y>","<z>","<d>","t"'

if(grafmag) then
open(5*c,file='mag_tempo.plt')
write(5*c,*)'Variables="Mx","My","Mz","M","t"'
end if
...
!******************* Distribuindo a partícula superior numa grade no plano superior *****************************!


! Distribuindo as primeiras n*(n-1) partículas superiores, em que n = (N-2)**1/3

dx= laux/((N-2)**(1.0/3.0)-1)
Xo= (l-laux)/2.0 
Yo= (l-laux)/2.0 

if(j.le.(((N-2)**(1.0/3.0)-1)*((N-2)**(1.0/3.0)))) then

X(j,N,1)= Xo + (0.5*(((j-(loop2*((N-2)**(1.0/3.0)-1)))*2.0)-1))*dx
X(j,N,2)= Yo + loop2*dx


reale2= j/((N-2)**(1.0/3.0)-1.0)
inteiro2= j/((N-2)**(1.0/3.0)-1)

if(reale2.ne.0.0)then
if(reale2.eq.inteiro2) then
loop2=loop2+1
end if
end if

! Distribuindo o restante das partículas superiores (da 31 em diante...)

else

X(j,N,1)= Xo + (j-(((N-2)**(1.0/3.0)-1)*((N-2)**(1.0/3.0)) + loop3*((N-2)**(1.0/3.0)) + 1))*dx
X(j,N,2)= Yo + ((2.0*loop3) + 1.0)*0.5*dx

reale2= j/((N-2)**(1.0/3.0))
inteiro2= j/((N-2)**(1.0/3.0))
...
X(j,N,3)= (h+laux)/2.0 + laux/2.0
DIAM(j,N)=100.0

!****************************************************************************************************************!

! Distribuindo a segunda partícula (de baixo) no centro do tabuleiro

X(j,N-1,1)= Xo + 0.5*laux
X(j,N-1,2)= Yo + 0.5*laux
X(j,N-1,3)= (h+laux)/2.0 + laux/4.0

DIAM(j,N-1)=10.0

! Distribuindo o restante das partículas do arranjo

do i=1,N-2

reale= (i)/((N-2)**(1.0/3.0))
inteiro= (i)/((N-2)**(1.0/3.0))

 a=i-loop*((N-2)**(1.0/3.0))
 b=1+loop-(auxiliar1)*((N-2)**(1.0/3.0))
 c=(i-1)/((N-2)**(2.0/3.0)) 
 auxiliar1=i/((N-2)**(2.0/3.0)) 
...
X(j,i,1)= Xo + (a-1)*dx !(laux-(laux/((N-2)**(1.0/3.0))))/(((N-2)**(1.0/3.0))-1.0)
X(j,i,2)= Yo + (b-1)*dx !(laux-(laux/((N-2)**(1.0/3.0))))/(((N-2)**(1.0/3.0))-1.0)
X(j,i,3)= (h-haux)/2.0 + (c)*dx !(laux-(laux/((N-2)**(1.0/3.0))))/(((N-2)**(1.0/3.0))-1.0)
...
! Atribuindo a mesma cor às partículas do arranjo
